...
The production 3D Home composites real WebGL worlds **and a real 3D room**
(window, lanterns, dust — see `src/components/three/RoomEnvironment.tsx`) over an
atmospheric background layer. These are the drop-in asset slots. **Every slot has
a robust fallback**, so the app builds and looks correct even with no `.webp`
present — drop a final asset in with the exact name to upgrade that layer.
...
Layering (back → front): warm dark base + CSS gradient fallback →
`home-background-*.webp` (or `home-ambience.svg`) on `.lab3d-atmosphere::after` →
**transparent Canvas** with the real 3D room + worlds → HTML UI overlay.

Do not bake fake worlds, buttons, or text into any background asset — the real
UI and real 3D always render on top. A missing `.webp` layer is simply empty and
reveals the fallback beneath it; nothing breaks.
